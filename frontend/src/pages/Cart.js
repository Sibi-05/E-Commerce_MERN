...
const Cart = () => {
  ...
  const calculateSubtotal = () => {
    return products.cartItems.reduce((total, item) => {
      // Ensure item?.product?.price?.org is a valid number, otherwise treat it as 0
      const price = item?.product?.price?.org || 0;
      // Ensure item?.quantity is a valid number, otherwise treat it as 0
      const quantity = item?.quantity || 0;
      // Add to total, multiplying price by quantity
                    ...
                    {products.cartItems?.map((item) => (
                      <Table>
                        ...
                        <TableItem>
                          {" "}
                          $
                          {(item.quantity * item?.product?.price?.org).toFixed(
                            2
                          )}
                        </TableItem>
                        <TableItem>
